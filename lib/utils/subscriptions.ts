...
export async function checkValidationQuota(userId: string, ideaId?: string): Promise<QuotaCheck> {
    ...
    // If this idea has already been validated THIS MONTH, it's free and doesn't use a credit
    if (ideaId) {
        ...
        if (existingThisMonth && existingThisMonth.length > 0) {
            return { allowed: true, remaining: 'unlimited', usingCredit: false }
        }
    }

    // Now check if user has free validation credits to use for a NEW monthly validation
    ...
    // Check if they have monthly quota left before suggesting a credit
    if (limits.validationsPerMonth !== 'unlimited') {
        ...
        const remainingQuota = Math.max(0, (limits.validationsPerMonth as number) - currentUniqueCount)

        if (remainingQuota > 0) {
            return { allowed: true, remaining: remainingQuota, usingCredit: false }
        }
    } else {
        return { allowed: true, remaining: 'unlimited', usingCredit: false }
    }

    // If quota is reached, then check for credits
 ...
 * Allows bypass using free validation credits if available.
 */
export async function checkIterationQuota(ideaId: string, userId: string): Promise<QuotaCheck> {
    const tier = await getUserTier(userId)
    const limits = TIER_LIMITS[tier]
    ...
    if (error) {
        return { allowed: false, remaining: 0, error: 'Failed to verify iteration quota.' }
    }
    ...
    if (currentCount < maxAllowed) {
        return { allowed: true, remaining: maxAllowed - currentCount, usingCredit: false }
    }

    // If at limit, check for credits to bypass
    const { data: karma } = await supabase
        .from('user_karma')
        .select('free_validation_credits')
        .eq('user_id', userId)
    ...
    return {
        allowed: false,
        remaining: 0,
        error: 'Iteration limit reached for this idea. Upgrade to Pro for unlimited iterations or use free credits.'
    }
}
